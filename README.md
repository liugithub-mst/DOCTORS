...
In addition to these required files, the user must provide the material type of the object, which describes the chemical element composition of each voxel. Two prebuilt material types are available—namely, water and human abdomen body part. Users can edit these examples for their own applications.

## How to Run DOCTORS
To run DOCTORS, go to the directory where the DOCTORS executable and the Python GUI are located, and type the following command:
...
To uninstall DOCTORS, simply delete the DOCTORS executable or the entire folder containing DOCTORS and the Python GUI files.

## Example Results
As a simple demonstration of the accuracy of DOCTORS, we show below the effective dose using MCNP6 and DOCTORS of an abdoman body part. The relative differnece between DOCTORS and MCNP6 is less than 5%, and DOCTORS is over 10 times faster than MCNP6. 
